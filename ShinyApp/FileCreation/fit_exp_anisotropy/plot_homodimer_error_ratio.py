...
names = ['Acetone (O)','Ar ()','Chloromethane (Cl)','CO$_2$ (C,O)',
            'Dimethyl Ether (O)','Ethane ()',
            'Ethanol (O,H)','Ethene (C,H)','H$_2$O (O,H)',
            'Methane ()','Methanol (O,H)','Methyl Amine (N,H)','NH$_3$ (N,H)']
...
for fit_file in fit_files:
    ...
    df = df.sort(['Total Energy'],ascending=False)
    vals = df.values
    #normal = mpl.colors.Normalize(vals.min(), 1.0)
    #normal = mpl.colors.Normalize(1.0,vals.max())
    #normal = mpl.colors.PowerNorm(1.0,vals.max())
    normal = mpl.colors.PowerNorm(.5,1.0,vals.max())
    # print normal
    ...
    plt.axis('off')
    fig.savefig('homodimer_error_ratios.pdf',bbox_inches='tight',dpi=100)
    plt.show()
