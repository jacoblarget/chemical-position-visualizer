...
fit_files = ['kt05_constrained_exp_coeffs_unconstrained.out',
                'kt20_constrained_exp_coeffs_unconstrained.out',
                'kt50_constrained_exp_coeffs_unconstrained.out',
                    ]
    ...
    for component in components:
        ...
        for imon1,mon1 in enumerate(molecules):
            for mon2 in molecules[imon1:]:
                ...
                rms_errors = [1000*float(i) for i in rms_errors.split()]
                weighted_rms_errors = [1000*float(i) for i in weighted_rms_errors.split()]

                rms_geomean.append(rms_errors[-1])
                weighted_rms_geomean.append(weighted_rms_errors[-1])
                
                print rms_errors[-1], weighted_rms_errors[-1]
