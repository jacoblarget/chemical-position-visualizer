...
first=True
labels = []
label_texts = itertools.cycle(['Slater-ISA FF','MASTIFF'])
color_order = itertools.cycle([1,0,2])
zorder = itertools.cycle([9,8,10])
m_values = []
b_values = []
for i, (component_prefix,component_suffix) in enumerate(zip(component_prefixes,component_suffixes)):
    ...
    for component in electrostatics,exchange,dispersion,None,induction,dhf,total_energy,total_energy:
        ...
        if last:
            ...
            if 'Total Energy' in titles[count-1]:
                ...
                plt.fill_between(x1,x1-kjmol,x1+kjmol,
                        edgecolor='None',
                        zorder=0,alpha=0.10,color='k')
            ## kjmol = 1.0
            ## plt.fill_between(x1,x1-kjmol,x1+kjmol,
            ##         edgecolor='None',
            ##         zorder=0,alpha=0.1,color='k')

        #if first and titles[count-1] == 'Electrostatics' :
        if first: 
            # Plot RMSE Header
            nprefixes = len(component_prefixes)
            props = dict(facecolor=sns.axes_style()['axes.facecolor'], lw=0, alpha=1)
            textstr = 'RMSE (aRMSE)'
            ax.text(0.93, 0.02+0.07*nprefixes, textstr, transform=ax.transAxes, 
                     fontsize=10,
                     color = 'k',
                     fontweight='bold',
        ...
        x1 = np.array(total_energy['qm'])
        cutoff = 0.0
        weighted_diff = diff[np.where(x1 < cutoff)]
        attractive_rms_error = np.sqrt(np.mean(weighted_diff))
        ...
        textstr = '{:6.3f} ({:.3f}) kJ/mol'.format(rms_error,attractive_rms_error)
        props = dict(facecolor=sns.axes_style()['axes.facecolor'], lw=0, alpha=1)
        nprefixes = len(component_prefixes)
        ax.text(0.98, 0.02 + 0.07*(nprefixes-i-1), textstr, transform=ax.transAxes, 
                 fontsize=12,
                 color = palette[i],
                 verticalalignment='bottom', horizontalalignment='right',bbox=props)
