...
fit_files = [
    ## ['scaledispanisotropic_fit_exp_total_energy_unconstrained.dat','scaledispisotropic_fit_exp_total_energy_unconstrained.dat'],
    ...
    ['isotropic_fit_exp_total_energy_unconstrained.dat','fullisotropic_fit_exp_total_energy_unconstrained.dat'],
    ## ['anisotropic_fit_exp_total_energy_unconstrained.dat','isotropic_fit_exp_total_energy_unconstrained.dat'],
...
for fit_file in fit_files:
    ...
    for imon1,mon1 in enumerate(molecules):
        ...
        for component in components:
            ...
            ifile1 = fit_file[0].replace('total_energy',component)
            ifile2 = fit_file[1].replace('total_energy',component)

            with open(dir_name + ifile1,'r') as f:
                data = [ line.split() for line in f.readlines()[1:] ]
                data1 = np.array(data,dtype=float)[:,1]
            with open(dir_name + ifile2,'r') as f:
                data = [ line.split() for line in f.readlines()[1:] ]
                data2 = np.array(data,dtype=float)[:,1]

            linear_thresh = 1e-3 # 1mH
            mape = np.mean(np.abs((data1 - data2)/(np.abs(data1)+1e-3))*100)
            weighted_data1 = data1[np.where(total_energy_data[:,0] < 0)]
            weighted_data2 = data2[np.where(total_energy_data[:,0] < 0)]
            weighted_mape = np.mean(np.abs((weighted_data1 - weighted_data2)/(np.abs(weighted_data1)+1e-3))*100)
            #weighted_mape = np.mean(np.abs((weighted_data1 - weighted_data2)/weighted_data2)*100)

            ## rmse_errors.append(rms_diff)
            ## weighted_rmse_errors.append(weighted_rms_diff)

            ## rmse_error_ratio = rmse_errors[0]/rmse_errors[1]
            ## weighted_rmse_error_ratio = weighted_rmse_errors[0]/weighted_rmse_errors[1]

            ## rms_diff *= 2625.5
            ## weighted_rms_diff *= 2625.5

            rms_ratios.append(mape)
            rms_ratios.append(weighted_mape)
