...
for line in data:
    if not line:
        ...
        tmp['mae'].append(float(line[1]))
        #tmp['stds'].append(float(line[3]))
        tmp['attractive_mae'].append(float(line[2]))
        #tmp['attractive_stds'].append(float(line[6]))
        ## tmp.append({ name : float(i) 
        ##     for name, i in zip(['mae','attractive_mae'],line[1:]})
...
for i,dset in enumerate(datasets):
    ...
    df = dfs[i]
    ## df['rms_pyerr'] = (df['mae']*df['stds'] - df['mae'])
    ## df['rms_nyerr'] = ( - df['mae']/df['stds'] + df['mae'])
    ## df['attractive_rms_pyerr'] = (df['attractive_mae']*df['attractive_stds']
    ##                                 - df['attractive_mae'])
    ## df['attractive_rms_nyerr'] = ( - df['attractive_mae']/df['attractive_stds']
    ##                                 + df['attractive_mae'])
    ## df['rms_pyerr'] = df.map(df['mae']*df['stds'])
    ## df['rms_nyerr'] = df.map(df['mae']/df['stds'])
    ## df['attractive_rms_pyerr'] = df.map(df['attractive_mae']*df['attractive_stds'])
    ## df['attractive_rms_nyerr'] = df.map(df['attractive_mae']/df['attractive_stds'])
    #print df[i]
...
offset = 0.00
#mae_colors = itertools.cycle(sns.color_palette('Paired',desat=0.75))
## mae_colors = itertools.cycle(sns.color_palette('Set2',desat=0.75))
## #attractive_mae_colors = itertools.cycle(sns.hls_palette(l=.3, s=.8))
## #attractive_mae_colors = itertools.cycle(sns.color_palette('Paired'))
## attractive_mae_colors = itertools.cycle(sns.color_palette('Set2'))
#names = itertools.cycle(names)
names = itertools.cycle(["Isotropic","Constrained Isotropic","Anisotropic ON","Constrained Anisotropic ON","Anisotropic OHN", "Constrained Anisotropic OHN"])
front_scale = 0.8
back_scale = 0.9
## mae_colors = sns.color_palette('Set2',desat=0.75)
## attractive_mae_colors = sns.color_palette('Set2')
mae_colors = sns.color_palette('Paired',desat=0.75)
attractive_mae_colors = sns.color_palette('Paired')
for i,dset in enumerate(dfs):
    ...
    #ax.bar(shift,dset['mae'],width*back_scale,color=mae_colors.next(),alpha=0.6,yerr=dset['stds'])
    ax.bar(shift,dset['mae'],width*back_scale,
            color=mae_colors[1-i],
            edgecolor=mae_colors[1-i],
            #color=mae_colors.next(),
            alpha=0.6,
    ...
    ax.bar(shift,dset['attractive_mae'],width*front_scale,
            color=attractive_mae_colors[1-i],
            edgecolor=attractive_mae_colors[1-i],
            label=names.next(),
...
m1, = ax.plot([], [], c=attractive_mae_colors[1] , marker='s', markersize=24,
                      fillstyle='full', linestyle='none')
m2, = ax.plot([], [], c=attractive_mae_colors[0] , marker='s', markersize=24,
                      fillstyle='full', linestyle='none')
m3, = ax.plot([], [], c=attractive_mae_colors[-1] , marker='s', markersize=24,
                      fillstyle='full', linestyle='none')
m4, = ax.plot([], [], c=attractive_mae_colors[-2] , marker='s', markersize=24,
                      fillstyle='full', linestyle='none')
m5, = ax.plot([], [], c=attractive_mae_colors[-3] , marker='s', markersize=24,
                      fillstyle='full', linestyle='none')
m6, = ax.plot([], [], c=attractive_mae_colors[-4] , marker='s', markersize=24,
                      fillstyle='full', linestyle='none')
...
with sns.axes_style('dark'):
    ...
    #leg.axis([0, 1, 0, 1])
    leg.axis('off')
## m1, = leg.plot([], [], c=mae_colors[0] , marker='s', markersize=15,
##                       fillstyle='left', linestyle='none',alpha=0.6)
## m2, = leg.plot([], [], c=mae_colors[1] , marker='s', markersize=15,
##                       fillstyle='right', linestyle='none',alpha=0.6)
## m3, = leg.plot([], [], c=attractive_mae_colors[0] , marker='s', markersize=15,
##                       fillstyle='left', linestyle='none')
## m4, = leg.plot([], [], c=attractive_mae_colors[1] , marker='s', markersize=15,
...
ax.set_ylabel('Average RMS Errors (mH)',fontsize=24,fontweight='bold')
...
fig.savefig('transferability_mae_errors.pdf',bbox_inches='tight',dpi=1200)
plt.show()
